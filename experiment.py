...
class Experiment(object):
    def __init__(self,
        ...
        dataloader_name = 'dataloader_' + str(vadataloader_param['idx'])
        import_str = "from src.dataloader.{0} import {0}".format(dataloader_name)
        exec(import_str, globals()) 
    ...
    def train(self):
        ...
        self.training_task.run_training(self.model, self.dataloader_train, self.loss, self.optimizer, 
                                        self.scheduler, self.dataloader_val, self.dataloader_test, self.eval_task, self.dataset_param_test, self.dataset_param_train, self.dataset_param_val, self.run_name)


    def evaluate(self):
        self.eval_task(self.model, self.dataset_param_val, self.dataloader_val, self.dataset_param_test, self.dataloader_test, self.run_name, self.dataloader_train.epochs)


    def test(self, model, device, dataloader, epoch):
        logging.info("Started evaluation for epoch: {}".format(epoch))
        desc = self.pbar.desc
        ...
        output =  np.zeros((num_unlab,512))
        targets = np.zeros((num_unlab,))
        for batch_idx, ((x1,x2),idx) in enumerate(dataloader()):
            x1, x2= x1.to(device, non_blocking=True), x2.to(device, non_blocking=True)
            with torch.no_grad():
                output1,feature1 = model(x1)
                output2, feature2 = model(x2)

            loss = self.loss(output1, output2, self.device)
            loss_record.update(loss.item(), output1.shape[0])

            passes += output1.shape[0]
            self.pbar.set_description(progress.format(passes ,num_unlab).center(50))

            output[idx,:] = feature1.cpu().numpy()
        acc = 0.0
        # TO DO
        #acc = kmeans(output,targets,num_unlabeled_classes=targets)
        # plot_umap(output, targets, epoch,targets)
    ...
    def save_experiment_config(self,
        ...
        config = {
            ...
            "vadataloader_param": self.vadataloader_param,
